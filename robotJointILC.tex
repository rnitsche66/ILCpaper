...
\begin{Remark}\label{rem:friction}
  ...
  the pneumatic joint with high accuracy. \hfill $\square$
\end{Remark}
...
\begin{Remark}\label{rem:stableILC}
  ...
  \begin{align}
    ...
    \end{align}
\end{subequations}
where $\I$ is the idedity matrix and $\rho$ is the spectral radius as
defined in \cite{Golub2013}. \hfill $\square$
...
corresponds to the tracking performance of the flatness-based
controller only. A maximal tracking error of $\approx 1.5^{\circ}$ can
be seen after the first run, \ie. for the controller without the ILC
contribution. As can be seen clearly after several runs, the tracking
error decreases significantly. It can be seen in
...
\begin{Remark}\label{rem:ILCsmallDrive}
  ...
  possible with any other control concept. \hfill $\square$
\end{Remark}
